...
object ActorFlow {
  ...
  def actorRef[In, Out](props: Function[ActorRef, Props], bufferSize: Int = 1000, overflowStrategy: OverflowStrategy = OverflowStrategy.dropNew, fanout: Boolean = false)(implicit factory: ActorRefFactory, mat: Materializer): Flow[In, Out, NotUsed] = {
    ...
    Flow.fromSinkAndSource(
      Sink.actorRef(
        factory.actorOf(Props.create(classOf[ActorFlow.WebsocketFlowActor], () => new WebsocketFlowActor(props, ref))),
        Status.Success("")
      ),
  ...
  private class WebsocketFlowActor(props: Function[ActorRef, Props], ref: ActorRef) extends Actor {
